...
function createHtmlIframeFilesForSection (sectionKey, section, options, pages) {
  ...
  if (sectionKey !== 'index') {
    ...
    var alternative2Page;
    if (section.alternative2Markup) {
      alternative2Page = {
        data: {
          layout: options.alternative2TemplateIframe
        },
        dest: options.dest + "/" + section.alternative2HtmlFile,
      };
...
function createHtmlIframeFilesForSubsections (section, options, pages) {
  ...
  for (var subSectionKey in section) {
    ...
    if (typeof(subSection) === 'object' && subSection.sectionName !== undefined) {
      ...
      var alternative2SubSectionPage;
      if (subSection.alternative2Markup) {
        alternative2SubSectionPage = {
          data: {
            layout: options.alternative2TemplateIframe
          },
          dest: options.dest + "/" + subSection.alternative2HtmlFile,
        };
      ...
      if (subSection.alternative2Markup) {
        pages.push(alternative2SubSectionPage);
      }
