...
kssCommentsParser.addSubSectionsToObject = function (subSections, parentSection, pathToRootSection) {
  'use strict';
  ...
  if (!parentSection[subSections[0]]) {
    ...
    if (parentSection.level === 0) {
      ...
      htmlFilePath = currentSectionNameWithoutWhitespace + '/' + currentSectionNameWithoutWhitespace + '.html';
    } else {
      ...
      htmlFilePath = pathToRootSection + '/' + currentSectionNameWithoutWhitespace + '.html';
    }
  ...
  if (pathToRootSection === '') {
    ...
    pathToRootSection = pathToRootSection + '/' + currentSectionNameWithoutWhitespace;
  }
  ...
  if (subSections.length > 1) {
    ...
    return kssCommentsParser.addSubSectionsToObject(remainingSections, currentSection, pathToRootSection);
  }
...
kssCommentsParser.getSectionObjectOfKssComment = function (cssCommentPathObject, sections, grunt) {
  ...
  if (sectionRegex.test(cssComment)) {
    ...
    var iframeHeight;
    if (sectionObject.properties && sectionObject.properties['iframe-height'] && sectionObject.properties['iframe-height'][0]) {
      iframeHeight = sectionObject.properties['iframe-height'][0];
