        ...
        if (markupAvailable) {
            ...
            if (markup.search(/.*\.hbs/gi) === 0) {
                ...
                if (sectionObject.properties && sectionObject.properties['json-file'] && sectionObject.properties['json-file'][0]) {
                    path = _getJSOJNPath(sectionObject.properties['json-file'][0], srcPath);
                } else {
                ...
                if (grunt.file.exists(path)) {
                    try {
                        ...
                        if (data) {
                            ...
                            sectionObject.markupContextPath = path;
                        }
                    } catch (exception) {
            ...
            if (grunt.file.exists(angularMarkupPath)) {
                ...
                try {
                    ...
                    if (angularMarkupData) {
                        ...
                        sectionObject.angularMarkupPath = angularMarkupPath;
                    }
                } catch (exception) {
            ...
            if (grunt.file.exists(path)) {
                console.log('angularContext ' + path);
                try {
                    data = grunt.file.readJSON(path);
                    if (data){
                        sectionObject.angularContext = data;
                        sectionObject.angularContextPath = path;
                    }
                } catch (exception) {
