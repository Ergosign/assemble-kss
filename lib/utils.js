...
exportingObject.findKssCommentsInDirectory = function (directory, sourceMask, grunt) {

    var returnArray = [];

    grunt.file.recurse(directory, function (abspath, rootdir, subdir, filename) {
        if (grunt.file.isDir(abspath)) {
            var directoryArray = exportingObject.findKssCommentsInDirectory(abspath, sourceMask, grunt);
...
var _addSubSectionsToObject = function (subSections, parentSection) {
...
exportingObject.getSectionObjects = function (foundKssCommentsPath, grunt){
    var sections = {
        level: 0
    };
